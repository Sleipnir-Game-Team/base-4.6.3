...
func pause() -> void:
	_pause_layers += 1
	SLogger.info("Camada de pause adicionada, atualmente existem %s camadas" % [_pause_layers])
...
func resume() -> void:
	_pause_layers -= 1
	SLogger.info("Camada de pause removida, atualmente existem %s camadas" % [_pause_layers])
