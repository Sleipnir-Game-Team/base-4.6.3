...
var max_audiostreams : int = 120           ## Número máximo de sons tocando ao mesmo tempo
...
var _currently_playing_size: int:          # NOTE nao sei se é o melhor lugar para isso estar
	...
	set(value):
		if currently_playing_audiostreams.size() > max_audiostreams:
			...
			SLogger.warn("Number of AudioStreamPlayers playing at the same time passed the maximum of "+
...
func _enter_tree() -> void:
	...
	SLogger.info("Available Audio Buses are: "+", ".join(audiobuses))
...
func play_global(sfx_name: String) ->void:
	if SfxGlobals.sound_events.has(sfx_name.to_lower()):  
		...
		SLogger.warn(sfx_name+" not yet implemented!")
...
func change_bus_volume(bus_name:String,value:float,allow_clipping:bool=false) ->void:
	...
	if bus_idx == -1:
		SLogger.warn(bus_name+" Bus don't exist")
		return 
...
func create_bus(bus_name:StringName) -> Error: 
	match _check_bus_dont_exist(bus_name):
		OK: # bus não existe
			...
			SLogger.info("Added: "+bus_name.to_lower()+" bus")
			return OK
			
		var result: # se bus existe bota o nome dele em uma variável result
			SLogger.warn("Cannot create \""+result+"\" bus, it already exists.")
			return ERR_ALREADY_EXISTS
		_:      # qualquer outra merda é um erro gigante
			SLogger.fatal("Erro inesperado ao criar bus")
			return FAILED
...
func remove_bus(bus_name:String) -> Error: 
	match _check_bus_dont_exist(bus_name):
		OK: # bus não existe
			SLogger.warn("Cannot remove \""+bus_name+"\" bus, it does not exist.")
			return ERR_DOES_NOT_EXIST
			
		var result: # se bus existe bota o nome dele em uma variável result
			AudioServer.remove_bus(AudioServer.get_bus_index(result))
			SLogger.info("Removed: \""+result+"\" bus")
			return OK
		_:  # qualquer outra merda é um erro gigante
			SLogger.fatal("Erro inesperado ao remover bus")
			return FAILED
...
func _stop(audio_player) -> void:  # O que realmente para o sfx
	if audio_player.has_method("stop_sound"): # parar o som
		...
		SLogger.error(str(audio_player)+" Not SoundQueue nor AudioStreamPlayer")
	...
	for bus in buses:
		match _check_bus_dont_exist(bus):
			OK:
				create_bus(bus)
			var result:
				SLogger.warn("\""+result+"\" bus already exists")
				continue
			_:
				SLogger.error("Error creating \""+bus+"\" bus")
				continue
	
	var bus_layout = AudioServer.generate_bus_layout()
	match ResourceSaver.save(bus_layout,"res://default_bus_layout.tres"):
		OK:
			SLogger.info("New bus layout loaded succesfully!")
			AudioServer.set_bus_layout(bus_layout)
			return OK
		_:
			SLogger.warn("Failed in loading bus layout")
