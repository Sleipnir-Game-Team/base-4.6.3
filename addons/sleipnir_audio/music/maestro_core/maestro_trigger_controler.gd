...
func _trigger_populate(triggers:Array) ->void: 
	...
	if SleipnirMaestro.log_level <=2 and SleipnirMaestro._trigger_stems.size() > 0: 
		SLogger.info("Trigger Loaded = "+str(SleipnirMaestro._trigger_stems.size()))
		SLogger.info("Loaded Triggers = "+str(SleipnirMaestro._trigger_stems))
	elif SleipnirMaestro.log_level <=2 and SleipnirMaestro._trigger_stems.size() == 0:
		SLogger.info("This Song Doesn't Have Any Trigger Stems")
...
func _trigger_play(TriggerStem:AudioStreamPlayer,sync_method:int=0) -> Error:
	if TriggerStem.is_queued_for_deletion(): # se vai ser liberado da tree
		if SleipnirMaestro.log_level <=2: SLogger.info("Ignoring trigger play for "+TriggerStem.name+", because it's being freed")
	...
	match sync_method:
		0: ## por beat
			...
			if SleipnirMaestro.log_level <=2:
				SLogger.info("playing: "+TriggerStem.name+" ["+str(SleipnirMaestro._get_elapsed_time())+"]")
		...
		_: ## fora da seleção
			if SleipnirMaestro.log_level <= 3: SLogger.warn("Invalid sync method for playing trigger, using next beat")
			await SleipnirMaestro.next_beat
			if !is_instance_valid(TriggerStem): return ERR_UNAUTHORIZED
