...
func _get_sync_streams(sync_stream:AudioStreamSynchronized) -> void:
	...
	if SleipnirMaestro.log_level <= 1:
		SLogger.info("there are " + str(count) + " sync_streams and they are: " +str(sync_streams))


func toggle_layer_on(SyncStream:int) -> Error:
	if sync_streams.is_empty():
		if  SleipnirMaestro.log_level <= 3: SLogger.warn("No sync_streams for this player")
		return ERR_DOES_NOT_EXIST
	
	if sync_streams[SyncStream] == true:
		if  SleipnirMaestro.log_level <=1: SLogger.debug("Layer Already Active!")
		return ERR_ALREADY_IN_USE
	
	if self.get_stream() is not AudioStreamSynchronized:
		if  SleipnirMaestro.log_level <= 3 : SLogger.warn("can't toggle unless is AudioStreamSynchronized")
	...
	if SleipnirMaestro.log_level<=1: 
		SLogger.debug("Stream of Number: "+str(SyncStream)+" from "+str(old)+" to "+str(new))
	if SleipnirMaestro.log_level<=1: 
		SLogger.debug("BeatsPerBar: "+str(SleipnirMaestro.BeatsPerBar)+
	...
	if SleipnirMaestro.log_level <=2 : SLogger.info("Toggled On "+str(self.stream.get_sync_stream(SyncStream)))
	return OK
...
func toggle_layer_off(SyncStream:int) -> Error:
	...
	if sync_streams[SyncStream] == false:
		if  SleipnirMaestro.log_level <=1: SLogger.debug("Layer Already Inactive!")
		return ERR_ALREADY_IN_USE
	...
	if SleipnirMaestro.log_level <=2 : SLogger.info("Toggled Off "+str(self.stream.get_sync_stream(SyncStream)))
	return OK
